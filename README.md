...
建议使用 Python 3.10 或更高版本。每个平台的启动脚本默认使用各自目录下的虚拟环境，淘宝也可以使用仓库根目录的虚拟环境。
...
- Walmart：先打开目标 AdsPower profile，并在同一 profile 中打开 `walmart.com` 标签页。程序只读取已打开浏览器的登录态，不绕过 CAPTCHA 或人工验证。
