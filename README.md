...
本仓库把淘宝、Walmart、Wayfair、Reddit 和 TikTok 五个独立爬虫放在同一个目录中，并提供统一的选择入口。平台抓取逻辑仍然相互独立；统一启动器只负责选择平台、检查目录和启动原有入口。
...
# TikTok（可使用 TikTok 自己的环境，也可复用根目录 .venv）
py -3 -m venv tiktok\.venv
.\tiktok\.venv\Scripts\python.exe -m pip install -r tiktok\requirements.txt
```
...
.\.venv\Scripts\python.exe unified_scraper.py --platform tiktok
...
.\.venv\Scripts\python.exe unified_scraper.py --platform tiktok --tiktok-dir D:\work\tiktok
```

也可以设置 `TAOBAO_SCRAPER_DIR`、`WALMART_SCRAPER_DIR`、`WAYFAIR_SCRAPER_DIR`、`REDDIT_SCRAPER_DIR`、`TIKTOK_SCRAPER_DIR` 环境变量；命令行参数优先于环境变量。淘宝运行时还可用 `--taobao-python` 指定 Python，Reddit 和 TikTok 还可用各自的 `--*-python` 指定 Python，Wayfair BAT 可用 `--cmd-exe` 指定 `cmd.exe`。
...
- TikTok：运行前在 AdsPower 中启动一个已打开 TikTok 的浏览器环境，并在 `tiktok/keywords.txt` 中填写关键词。统一启动器直接调用 Python 入口，由采集器自动发现 AdsPower 中的 TikTok 页面；独立 `tiktok/start_tiktok.bat` 会先通过 CDP 调试端口探测脚本找到页面。可选的 `ADSPOWER_API_KEY` 只从进程环境读取，不能写入仓库。
...
tiktok/
  tiktok_collector.py    # TikTok AdsPower/CDP 采集器
  find_adspower_port.ps1 # 独立 BAT 的 CDP 端口探测
  start_tiktok.bat       # ASCII-only TikTok 入口
  keywords.txt           # 安全示例关键词
  requirements.txt
```

各平台的 `runtime`、`taobao_runtime`、输出目录、结果文件、原始 HTML/JSON、断点、Cookie、登录态、浏览器 session、虚拟环境、缓存和生成的 `egg-info` 均被 `.gitignore` 排除。Reddit 的 `input/` 默认忽略新增文件，只保留 `keywords.txt`、`urls.txt` 两个无账号示例；TikTok 的 `data/`、`capture_data/` 和私有关键词文件也不应提交。请勿把真实 URL/关键词列表、账号信息或运行产物加入提交。仓库不包含任何实际账号、密码、Token、Cookie 或其他认证状态；请勿把这些内容手动加入提交。
...
py -3 -m compileall -q unified_scraper.py walmart\run.py walmart\src taobao\taobao_scraper wayfair\wayfair_http_scraper.py wayfair\embed_wayfair_images.py reddit\app.py reddit\reddit_scraper tiktok\tiktok_collector.py
