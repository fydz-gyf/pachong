...
class ExcelExporter:
    ...
    def _style(self, ws, row_count: int):
        ...
        widths = [
            10, 8, 10, 10, 18, 18, 55, 12, 12, 20, 24,
            10, 12, 18, 25, 16, 45, 10, 18, 14, 45,
        ]
        for i, width in enumerate(widths, start=1):
            ws.column_dimensions[get_column_letter(i)].width = width

        ws.freeze_panes = "A2"
        ws.auto_filter.ref = f"A1:U{max(1, row_count + 1)}"
        ws.row_dimensions[1].height = 34
        for row in range(2, row_count + 2):
            ws.row_dimensions[row].height = max(
                32,
                self.settings.image_max_size * 0.78 if self.settings.embed_images else 32,
            )
            ...
            ws[f"Q{row}"].alignment = Alignment(vertical="center", wrap_text=True)
            if str(ws[f"R{row}"].value).upper() == "Y":
                ws[f"R{row}"].fill = ad_fill
    ...
    def export(self, all_results: dict[str, list[dict]], environment: dict):
        ...
        for keyword, products in all_results.items():
            ws = wb.create_sheet(sanitize_sheet_name(keyword, used))
            ws.append(HEADERS)
            image_map = self.images.download_keyword(
                keyword,
                products,
                environment.get("userAgent", default_user_agent()),
            )
            if self.settings.embed_images:
                cleanup.append(self.settings.paths.images / safe_filename(keyword))
            ...
            for row_idx, p in enumerate(products, start=2):
                ws.append(
                    [
                        ...
                        to_number(p.get("sorftime_month_sales", "")),
                        to_number(p.get("sorftime_month_revenue", "")),
                        to_number(p.get("rating", "")),
                        to_number(p.get("review_count", "")),
                        p.get("brand", ""),
                        p.get("seller", ""),
                        p.get("availability", ""),
                        p.get("fulfillment", ""),
                        "Y" if p.get("is_sponsored") else "N",
                        p.get("raw_type", ""),
                        "",
                        "",
                    ]
                )
                self._set_link(ws[f"T{row_idx}"], p.get("product_url", ""), "查看商品")
                self._set_link(ws[f"U{row_idx}"], p.get("image_url", ""), "查看原图")
                image_path = image_map.get(str(p.get("item_id", "")), "")
                if image_path and os.path.exists(image_path):
                    try:
                        img = ExcelImage(image_path)
                        img.width = self.settings.image_max_size
                        img.height = self.settings.image_max_size
                        img.anchor = f"E{row_idx}"
                        ws.add_image(img)
                    except Exception as e:
                        logging.warning(
                            "Excel image embed failed item_id=%s: %s",
                            p.get("item_id"),
                            e,
                        )
