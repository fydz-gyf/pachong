...
"""Small dependency-free DES-CBC decryptor used for Sorftime response envelopes.

This module intentionally implements only what the scraper needs: DES decryption,
CBC chaining, and PKCS#7 removal. It avoids adding a new binary dependency to the
...
def _rot28(v: int, n: int) -> int:
    return ((v << n) | (v >> (28 - n))) & 0x0FFFFFFF


def _subkeys(key8: bytes) -> list[int]:
    if len(key8) != 8:
    ...
    for shift in SHIFTS:
        c = _rot28(c, shift)
        d = _rot28(d, shift)
        out.append(_permute((c << 28) | d, PC2, 56))
    return out
...
def decrypt_block(block8: bytes, key8: bytes) -> bytes:
    ...
    v = _permute(int.from_bytes(block8, "big"), IP, 64)
    l = (v >> 32) & 0xFFFFFFFF
    r = v & 0xFFFFFFFF
    for sk in reversed(_subkeys(key8)):
        l, r = r, l ^ _f(r, sk)
    out = _permute((r << 32) | l, FP, 64)
    return out.to_bytes(8, "big")


def cbc_decrypt(ciphertext: bytes, key8: bytes, iv8: bytes) -> bytes:
    if len(key8) != 8 or len(iv8) != 8:
        raise ValueError("DES key and IV must both be 8 bytes")
    if len(ciphertext) % 8:
        raise ValueError("DES ciphertext length must be a multiple of 8")
    subkeys = _subkeys(key8)

    def dec_block_fast(block8: bytes) -> bytes:
        v = _permute(int.from_bytes(block8, "big"), IP, 64)
        l = (v >> 32) & 0xFFFFFFFF
        r = v & 0xFFFFFFFF
